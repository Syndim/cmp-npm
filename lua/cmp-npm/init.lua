...
local is_windows = string.find(system, 'Windows') ~= nil

local npm_cmd    = is_windows and 'cmd.exe' or 'npm'
local npm_args   = is_windows and { '/c', 'npm.cmd' } or {}

local function resolve_args(args)
    local result = vim.tbl_extend('force', {}, npm_args)
    return vim.list_extend(result, args)
end
    ...
    if find_version then
        if opts.only_latest_version then
            Job
                :new({
                    command = npm_cmd,
                    args = resolve_args({ 'info', name, 'version' }),
            ...
            Job
                :new({
                    command = npm_cmd,
                    args = resolve_args({ 'info', name, 'versions', '--json' }),
                    on_exit = function(job)
                        local result = job:result()
                        ...
                        -- unfortunately, nvim-cmp uses its own sorting algorithm which doesn't work for semantic versions
        ...
        Job
            :new({
                command = npm_cmd,
                args = resolve_args({ 'search', '--no-description', '--parseable', name }),
                on_exit = function(job)
                    local result = job:result()
                    local items = {}
                    for _, npm_item in ipairs(result) do
